...
#define nwy_dbg_log(...) LOGData(TAG_SENSOR, __VA_ARGS__)
...
void HandleSensorData(SensorTypeEnum sensorIndex, const char* data, uint16_t dataLen, uint8_t dataType, SensorIntervalTypeEnum intervalType, uint16_t timeout)
{
    ...
    for(int i = 0; i < MAX_SENSORS; i++)
    {
        if(SensorData[i].SensorType == sensorIndex && SensorData[i].IsActive)
        {
            ...
            SensorData[i].SensorData[dataLen < sizeof(SensorData[i].SensorData) ? dataLen : sizeof(SensorData[i].SensorData) - 1] = '\0';
...
// Format: $SINT,IMEI,Date,Time,Lat,Long,Speed,Altitude,HDOP,Heading,IGN,SOS,SensorType,SensorDataType,{SensorData}*CS\r\n
    ...
    for(int i = 0; i < MAX_SENSORS; i++)
    {
           ...
           SensorData[i].IntervalType == SENSOR_INTERVALTYPE_INTERRUPT)
        {
            ...
            if(SensorData[i].DataType == SENSOR_DATATYPE_ASCII)
                strncat(databuf, (char*)SensorData[i].SensorData, SensorData[i].DataLen);
            else
            {
...
// Format: $SENS,IMEI,Date,Time,Lat,Long,Speed,Altitude,HDOP,Heading,IGN,SOS,NoofSensors,Sensor1Type,Sensor1DataType,{Sensor1Data},...*CS\r\n
uint8_t MakeSensorPacket(char* databuf, uint16_t bufferSize)
{
    ...
    for(int i = 0; i < MAX_SENSORS; i++)
    {
           ...
           SensorData[i].IntervalType == SENSOR_INTERVALTYPE_REGULAR)
        {
            ...
            strcat(sensorDataSection, "{");
            if(SensorData[i].DataType == SENSOR_DATATYPE_ASCII)
                strncat(sensorDataSection, (char*)SensorData[i].SensorData, SensorData[i].DataLen);
            else
            {
                // For HEX data type, convert to hex string
                for(uint8_t j = 0; j < SensorData[i].DataLen; j++)
...
uint8_t ProcessSensors(void)
{
    ...
    while(HasPendingInterrupt())
    {
        ...
        if(sensorType)
        {
            ...
            {
                if(TCPSocket_SendString(&ServerSocket[2], sensorPacketBuffer))
                {
                    sendSuccess = 1;
                    packetsSent++;
                }
    ...
    if(IsPacketReady.IsSensPacket)
    {
        ...
        if(HasPendingRegularData())
        {
            if(MakeSensorPacket(sensorPacketBuffer, sizeof(sensorPacketBuffer)))
            {
                ...
                if(ServerSocket[2].SocketState == SOCKET_CONNECTED)
                {
                    TCPSocket_SendString(&ServerSocket[2], sensorPacketBuffer);
                    packetsSent++;
                }
                else
                {
...
uint8_t ParseSensorData(const char* data, uint16_t dataLen)
{
    ...
    nwy_dbg_log("ParseSensorData: dataLen=%d, first char=0x%02X, data='%s'", dataLen, (uint8_t)data[0], data);
...
// Save SensorsConfig to flash storage (wrapped to SaveToFlash)
void SaveSensorConfigToFlash(void)
{
    SaveToFlash(SENSOR_CONFIG_PATH, &SensorsConfig, sizeof(SensorsTypedef));
}

// Load SensorsConfig from flash storage (wrapped to LoadFromFlash)
// Returns: 1 if loaded successfully, 0 if file not found or invalid (defaults used)
uint8_t LoadSensorConfigFromFlash(void)
{
    return LoadFromFlash(SENSOR_CONFIG_PATH, &SensorsConfig, sizeof(SensorsTypedef), InitSensors);
}
...
uint8_t ParseCLSResponse(const char* data, uint16_t dataLen)
{
    ...
    // Valid CLS response - store entire response as-is
