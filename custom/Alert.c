...
AlertTypedef VAlert[ALERT_COUNT] = { {0} };

StoreAlertTypedef StoredAlert={0};

#ifndef PROTO_CDAC
char alertACK[MAX_ACK_SIZE]={0};
#endif
...
void AlertInitStruct(void)			//AlertInitStruct
{
	...
	{
		VAlert[i].Enable=0;
		Ql_memset(VAlert[i].ACK,0x00,sizeof(VAlert[i].ACK));
		Ql_strcpy(VAlert[i].ID,VTAlertPKT[i]);
		VAlert[i].ContMode = VTContType[i];
		switch(VTAlertHeaderType[i]){
			case 0:
			Ql_strcpy(VAlert[i].Header,"EPB");
			break;
			case 1:
			Ql_strcpy(VAlert[i].Header,"CRT");
			break;
			case 2:
			Ql_strcpy(VAlert[i].Header,"ALT");
			break;
			case 3:
			Ql_strcpy(VAlert[i].Header,"ACK");
			break;
			default:
			Ql_strcpy(VAlert[i].Header,"INV");
			break;
		}
	}
...
void RemoveAlert(uint8_t alert)
{
	...
	if(Ql_strlen(VAlert[alert].ACK)>0)
		Ql_memset(VAlert[alert].ACK,0x00,sizeof(VAlert[alert].ACK));
...
void AlertInitStruct(void)			//AlertInitStruct
{
	...
	for(i=0;i<ALERT_COUNT;i++)
	{
		VAlert[i].Enable=0;
		VAlert[i].Index=i;
		Ql_memset(alertACK,0x00,MAX_ACK_SIZE);
		Ql_strcpy(VAlert[i].ID,VTAlertPKT[i]);
		if(i<2)
		{
			Ql_strcpy(VAlert[i].Header,"EPB");
