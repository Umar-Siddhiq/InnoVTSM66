...
uint8_t SaveToFlash(char *filename, void *data, u32 size) {
  ...
  if (fd < 0) {
    LOGData(TAG_FILE, "File %s create failed, ret : %d, Formatting UFS",
            filename, fd);
...
uint8_t LoadFromFlash(char *filename, void *data, u32 size,
...
void LoadState(void) {
  LoadFromFlash(STATE_FILE_PATH, &VTSState, sizeof(VTSStateTypedef),
                LoadDefaultState);
}
