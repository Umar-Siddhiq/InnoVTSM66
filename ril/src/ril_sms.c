...
static bool LIB_SMS_ConvAlphaToCharSet_Fixed(u8 uDCS, u8 *pSrc, u16 uSrcLen, LIB_SMS_CharSetEnum eCharSet, u8 *pDest, u16 *pDestLen);
...
/******************************************************************************
* Function:     LIB_SMS_ConvAlphaToCharSet_Fixed
*  
* Description:
*               Fixed version of LIB_SMS_ConvAlphaToCharSet that properly converts
*               GSM 7-bit encoding to ASCII. The original library function has bugs
*               where it doesn't convert certain GSM 7-bit characters properly.
*
* Parameters:    
*               <uDCS>:
...
*               <eCharSet>
*                    [In] Character set (LIB_SMS_CHARSET_GSM, etc.)
*               <pDest>
*                    [Out] The destination buffer
*               <pDestLen>
*                    [In/Out] Buffer size in, actual length out
*
* Return:  
*               TRUE:  Success
*               FALSE: Failure
*
* NOTE:
*               GSM 7-bit Default Alphabet mapping (3GPP TS 23.038)
******************************************************************************/
static bool LIB_SMS_ConvAlphaToCharSet_Fixed(u8 uDCS, u8 *pSrc, u16 uSrcLen, LIB_SMS_CharSetEnum eCharSet, u8 *pDest, u16 *pDestLen)
{
    // GSM 7-bit default alphabet table (3GPP TS 23.038)
    static const u8 gsm7bit_to_ascii[128] = {
        '@',  0xA3, '$',  0xA5, 0xE8, 0xE9, 0xF9, 0xEC, // 0x00-0x07
        0xF2, 0xC7, '\n', 0xD8, 0xF8, '\r', 0xC5, 0xE5, // 0x08-0x0F
        '?',  '_',  '?',  '?',  '?',  '?',  '?',  '?',  // 0x10-0x17 (0x11 = underscore)
        '?',  '?',  '?',  '?',  0xC6, 0xE6, 0xDF, 0xC9, // 0x18-0x1F
        ' ',  '!',  '"',  '#',  0xA4, '%',  '&',  '\'', // 0x20-0x27
        '(',  ')',  '*',  '+',  ',',  '-',  '.',  '/',  // 0x28-0x2F
    ...
    if (NULL == pSrc || NULL == pDest || NULL == pDestLen || 0 == *pDestLen)
    {
        return FALSE;
    }
    
    // For GSM charset, convert using the proper table
    if (eCharSet == LIB_SMS_CHARSET_GSM)
    {
        for (i = 0; i < uSrcLen && destIdx < (*pDestLen - 1); i++)
        {
            u8 ch = pSrc[i];
            
            // Handle escape character for extended GSM 7-bit
            if (ch == 0x1B && (i + 1) < uSrcLen)
            {
                i++; // Skip escape char
                ch = pSrc[i];
                ...
                switch (ch)
                {
                    case 0x0A: pDest[destIdx++] = '\f'; break;  // Form feed
                    case 0x14: pDest[destIdx++] = '^'; break;
                    case 0x28: pDest[destIdx++] = '{'; break;
                    case 0x29: pDest[destIdx++] = '}'; break;
                    case 0x2F: pDest[destIdx++] = '\\'; break;
                    case 0x3C: pDest[destIdx++] = '['; break;
                    case 0x3D: pDest[destIdx++] = '~'; break;
                    case 0x3E: pDest[destIdx++] = ']'; break;
                    case 0x40: pDest[destIdx++] = '|'; break;
                    case 0x65: pDest[destIdx++] = 0xA4; break; // Euro sign
                    default: pDest[destIdx++] = '?'; break;    // Unknown extended
                }
            }
            else if (ch < 128)
            {
                pDest[destIdx++] = gsm7bit_to_ascii[ch];
            }
            else
            {
                pDest[destIdx++] = '?'; // Invalid character
            }
        }
    ...
    {
        // For other charsets, call the original library function
        return LIB_SMS_ConvAlphaToCharSet(uDCS, pSrc, uSrcLen, eCharSet, pDest, pDestLen);
    }
}
...
static bool ConvSMSParamToTextInfo(u8 uCharSet,LIB_SMS_PDUParamStruct *pSMSParam,ST_RIL_SMS_TextInfo* pTextInfo)
{
    ...
    if(LIB_SMS_PDU_TYPE_DELIVER == (pTextInfo->type))
    {
        ...
        bResult = LIB_SMS_ConvAlphaToCharSet_Fixed(
        ...
        if(FALSE == bResult)
        {
            DBG_TRACE(sg_aDbgBuf,"Enter ConvSMSParamToTextInfo,FAIL! LIB_SMS_ConvAlphaToCharSet_Fixed FAIL!");
            return FALSE;
        }
    }
...
static s32 SMS_CMD_GeneralHandler(char* pLine, u32 uLen, void* pUserData)
{
    ...
    {
        LOGData("SMS_HND","Enter SMS_CMD_GeneralHandler,FAIL! Parameter is NULL.");
    ...
    switch(uType)
    {
        case HDLR_TYPE_CPMS_READ_CMD:
        {
            ...
            {
                LOGData("SMS_HND","Enter SMS_CMD_GeneralHandler,SUCCESS. uType:HDLR_TYPE_CPMS_READ_CMD,storage:%u,used:%u,total:%u",(pInfo->storage),(pInfo->used),(pInfo->total));
        ...
        case HDLR_TYPE_CPMS_SET_CMD:
        {
            ...
            {
                LOGData("SMS_HND","Enter SMS_CMD_GeneralHandler,SUCCESS. uType:HDLR_TYPE_CPMS_SET_CMD,used:%u,total:%u",(pInfo->used),(pInfo->total));
                return RIL_ATRSP_CONTINUE;
            }
        }
        break;
        ...
        case HDLR_TYPE_CMGR_PDU_CMD:
        {
            ...
            {
                LOGData("SMS_HND","Enter SMS_CMD_GeneralHandler,SUCCESS. uType:HDLR_TYPE_CMGR_PDU_CMD,status:%u,length:%u",(pInfo->status),(pInfo->length));
                return RIL_ATRSP_CONTINUE;
            }
        }
        break;
        ...
        case HDLR_TYPE_CMGS_PDU_CMD:
        {
            ...
            {
                LOGData("SMS_HND","Enter SMS_CMD_GeneralHandler,SUCCESS. uType:HDLR_TYPE_CMGS_PDU_CMD,length:%u",(pInfo->pduLen));
                return RIL_ATRSP_CONTINUE;
            }
        }
        break;
        ...
        default:
        {
            LOGData("SMS_HND","Enter SMS_CMD_GeneralHandler,SUCCESS. Warning: NOT support type! uType:%u",uType);
        }
        break;
    }
    ...
    LOGData("SMS_HND","Enter SMS_CMD_GeneralHandler,SUCCESS. uType:%u",uType);
...
static s32 CmdSendPDUMsg(char* pPDUStr,u32 uPDUStrLen,u32 *pMsgRef)
{
    ...
    {
        LOGData("SMS_RIL","Enter CmdSendPDUMsg,FAIL! Parameter is NULL.");
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if(FALSE == bResult)
    {
        LOGData("SMS_RIL","Enter CmdSendPDUMsg,FAIL! LIB_SMS_IsValidHexStr FAIL!");
        return RIL_AT_INVALID_PARAM;
    }

    if((uPDUStrLen % 2) != 0)
    {
        LOGData("SMS_RIL","Enter CmdSendPDUMsg,FAIL! (uPDUStrLen % 2) != 0");
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if(iResult != RIL_ATRSP_SUCCESS)
    {
        LOGData("SMS_RIL","Enter CmdSendPDUMsg,FAIL! CMD: %s execute FAIL!",aCmd);
        return iResult;
    }

    LIB_SMS_SET_POINTER_VAL(pMsgRef,(sInfo.mr));

    LOGData("SMS_RIL","Enter CmdSendPDUMsg,SUCCESS. mr:%u",(sInfo.mr));
...
s32 RIL_SMS_SendSMS_Text_Ext(char* pNumber, u8 uNumberLen, LIB_SMS_CharSetEnum eCharset, u8* pMsg, u32 uMsgLen,u32 *pMsgRef,ST_RIL_SMS_SendExt *pExt)
{
    ...
    if((NULL == pNumber) || (0 == uNumberLen) || (NULL == pMsg))
    {
        LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text_Ext,FAIL! Parameter is NULL.");
        return RIL_AT_INVALID_PARAM;
    }

    if(FALSE == LIB_SMS_IS_SUPPORT_CHARSET(eCharset))
    {
        LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text_Ext,FAIL! LIB_SMS_IS_SUPPORT_CHARSET FAIL! eCharset:%u",eCharset);
        return RIL_AT_INVALID_PARAM;
    }
    ...
    if((pExt != NULL) && (TRUE == pExt->conPres))
    {
        ...
        {
            LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text_Ext,WARNING! msgType:%d is INVALID,use default!",pExt->con.msgType);
            (pExt->con.msgType) = LIB_SMS_UD_TYPE_CON_DEFAULT;
        }

        if(FALSE == IsValidConParam(&(pExt->con)))
        {
            LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text_Ext,FAIL! IsValidConParam FAIL!");
            return RIL_AT_INVALID_PARAM;
        }
    }
    ...
    if(NULL == pParam)
    {
        LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text_Ext,FAIL! Ql_MEM_Alloc FAIL! size:%u",sizeof(LIB_SMS_PDUParamStruct));
        return RIL_AT_FAILED;
    }
    ...
    if(NULL == pInfo)
    {
        ...
        LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text_Ext,FAIL! Ql_MEM_Alloc FAIL! size:%u",sizeof(LIB_SMS_PDUInfoStruct));
        return RIL_AT_FAILED;
    }
    ...
    if(FALSE == bResult)
    {
        ...
        LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text,FAIL! ConvStringToPhoneNumber FAIL!");
        return RIL_AT_FAILED;
    }
    ...
    if(FALSE == bResult)
    {
        ...
        LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text,FAIL! LIB_SMS_ConvCharSetToAlpha FAIL!");
        return RIL_AT_FAILED;
    }
    ...
    if(FALSE == bResult)
    {
        ...
        LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text,FAIL! LIB_SMS_EncodeSubmitPDU FAIL!");
        return RIL_AT_FAILED;
    }
    ...
    if(NULL == pPDUStr)
    {
        ...
        LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_Text,FAIL! Ql_MEM_Alloc FAIL! size:%d",((pInfo->uLen) * 2));
        return RIL_AT_FAILED;
    }
    ...
    if(FALSE == bResult)
    {
        ...
        LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_PDU,FAIL! LIB_SMS_CHECK_SUBMIT_PDU_STR_FOR_SEND FAIL!");
        return RIL_AT_INVALID_PARAM;
    }

    iResult = CmdSendPDUMsg(pPDUStr,uPDUStrLen,pMsgRef);

    LOGData("SMS_RIL","Enter RIL_SMS_SendSMS_PDU,SUCCESS. iResult:%d",iResult);
